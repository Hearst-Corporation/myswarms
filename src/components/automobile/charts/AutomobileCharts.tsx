  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
      <svg width={DONUT_SIZE} height={DONUT_SIZE} viewBox={`0 0 ${DONUT_SIZE} ${DONUT_SIZE}`}>
        ...
        <text x={cx} y={cx - 4} textAnchor="middle" fontSize={30} fontWeight={800} fill="var(--color-content-strong)">
          {total}
        </text>
        <text x={cx} y={cx + 16} textAnchor="middle" fontSize={FONT.nano} letterSpacing={LETTER_SPACING.mid} fill="var(--color-content-muted)">
          ANALYSES
        </text>
      </svg>
      <div style={{ display: "flex", flexDirection: "column", gap: 7, minWidth: 100 }}>
        {slices.map((s) => (
          <div key={s.key} style={{ display: "flex", alignItems: "center", gap: 7, fontSize: FONT.sm }}>
            ...
            <span style={{ color: "var(--color-content-muted)", flex: 1 }}>{s.label}</span>
            <strong style={{ color: "var(--color-content-strong)" }}>{s.count}</strong>
          </div>
        ))}
      </div>
    </div>
...
function Bars({ slices }: { slices: ChartSlice[] }) {
  if (slices.length === 0) {
    return <p className="text-sm text-content-faint">Aucune décision posée pour l&apos;instant.</p>;
  }
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      {slices.map((s, i) => (
        <div key={s.key} style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ width: 78, fontSize: FONT.xxs, color: "var(--color-content-muted)", flexShrink: 0 }}>{s.label}</span>
          <div style={{ flex: 1, height: 10, background: "var(--color-surface-3)", borderRadius: 999, overflow: "hidden" }}>
            <div
          ...
          <strong style={{ width: 24, textAlign: "right", fontSize: FONT.sm, color: "var(--color-content-strong)" }}>
            {s.count}
          </strong>
        </div>
      ))}
...
function PriceVsMarket({ rows }: { rows: PriceMarketRow[] }) {
  if (rows.length === 0) {
    return <p className="text-sm text-content-faint">Pas encore de cote marché pour les véhicules analysés.</p>;
  }
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      {rows.map((r, idx) => {
        ...
        const dotColor = over ? "var(--color-danger)" : deal ? "var(--color-ok)" : "var(--color-accent-strong)";
        ...
        return (
          <div key={`${r.label}-${idx}`}>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: FONT.xxs, marginBottom: 5 }}>
              <span style={{ color: "var(--color-content-strong)", fontWeight: FONT_WEIGHT.semibold }}>{r.label}</span>
            ...
            <div style={{ position: "relative", height: 16 }}>
              {/* axe */}
              <div style={{ position: "absolute", top: 7, left: 0, right: 0, height: 2, background: "var(--color-surface-3)", borderRadius: 2 }} />
              {/* bande P15–P85 */}
              <div
                style={{
                  position: "absolute",
              ...
              />
              {/* médiane */}
              <div style={{ position: "absolute", top: 1, left: `${pct(r.median)}%`, width: 2, height: 14, background: "var(--color-content-muted)" }} />
              {/* prix annonce */}
              <div
                className="av-dot"
                style={{
      ...
      <div style={{ display: "flex", gap: 14, fontSize: FONT.nano, color: "var(--color-content-faint)", flexWrap: "wrap" }}>
        <span>● <span style={{ color: "var(--color-ok)" }}>sous le marché</span></span>
        <span>● <span style={{ color: "var(--color-accent-strong)" }}>dans la fourchette</span></span>
        <span>● <span style={{ color: "var(--color-danger)" }}>au-dessus</span></span>
      </div>
    </div>
  );
}
...
function TokensArea({ series }: { series: number[] }) {
  if (series.length < 2) {
    return <p className="text-sm text-content-faint">Pas assez de runs pour la tendance tokens.</p>;
  }
  const W = 280;
  const H = 90;
    ...
    <svg width="100%" viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" style={{ display: "block" }}>
      <defs>
        <linearGradient id="av-tok" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="var(--color-accent-strong)" stopOpacity="0.45" />
      ...
      />
      {pts.map(([x, y], i) => (
        <circle key={i} cx={x} cy={y} r={2.4} fill="var(--color-accent-strong)" />
      ))}
    </svg>
  );
}
